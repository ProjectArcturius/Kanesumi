...
| §22 TwoPaneView | MetroTwoPaneView | `two_pane_view.rs` | MinWide 641/MinTall 641、宽切 Wide（L-R/R-L）、高切 Tall（T-B/B-T）、否则 SinglePane、pane1_ratio 分栏、纯布局容器 | ✅ | ✅ `defaults_are_uwp` / `narrow_is_single_pane_pane1` / `wide_splits_horizontally` / `wide_right_left_swaps` / `tall_splits_vertically` / `ratio_adjusts_split` / `pane_priority_selects_single` / `config_single_pane_forced` |
| §23 TitleBar | MetroTitleBar | `title_bar.rs` | Compact 32 / Expanded 48、Back 44×H（chevron 16）、Icon 16px（Margin 4）、Title 12px（Margin 16,0,16,2）、Activated/Deactivated 前景、Back hover/press tint | ✅ | ✅ `heights` / `back_rect_when_enabled` / `icon_rect_offsets` / `title_rect_after_icon` / `hit_back_only` / `deactivated_dims_foreground` / `back_hover_emits_tint` |
| §24 RatingControl | MetroRatingControl | `rating_control.rs` | MaxRating 5、star 24×24 间距 4、实星 ★ 强调色/空星 ☆、小数部分星裁剪、PointerOver 预览、IsClearEnabled 清零、IsReadOnly | ✅ | ✅ `defaults` / `star_geometry` / `click_sets_value` / `clear_enabled_resets` / `read_only_no_change` / `click_same_value_no_change` / `hover_sets_preview` / `render_emits_stars` |
| §25 TabView | MetroTabView | `tab_view.rs` | Item MinH 32、MinW 100/MaxW 240、等宽分配、Header Padding 0,8,0,0、Close 32×24、Add 32×24、选中底 surface_variant、超宽滚动 | ✅ | ✅ `tab_width_clamped` / `tab_geometry` / `close_rect_on_hovered` / `click_selects_tab` / `click_close_removes` / `click_add_hit` / `scroll_clamps` / `render_emits_tabs_and_add` / `selected_tab_has_surface_bg` |
| §26 TeachingTip | MetroTeachingTip | `teaching_tip.rs` | 面板 MinW 320/MaxW 336、ContentMargin 12、Title 14 SemiBold、Close 40×40、Row2 [Action][Close]、Tail 三角、四方向放置、淡入 0.167s/淡出 0.083s | ✅ | ✅ `placement_bottom_by_default` / `placement_top_when_no_bottom` / `placement_right_when_vertical_tight` / `open_sets_placement` / `close_fades_then_hides` / `hit_action_and_close` / `handle_click_closes` / `panel_height_grows_with_content` / `render_emits_when_open` / `closed_renders_nothing` |
| §27 TreeView | MetroTreeView | `tree_view.rs` | Item MinH 28、缩进 depth×16、chevron 16px（折叠右/展开下，翻转 0.1s）、Selected/PointerOver 白 15%、展平可见行、折叠隐藏子项 | ✅ | ✅ `flattened_rows_collapsed` / `expand_reveals_children` / `select_returns_path` / `toggle_via_chevron` / `indent_scales_with_depth` / `render_emits_rows_and_chevrons` / `collapsed_hides_subtree` |
| §28 NavigationView | MetroNavigationView | `navigation_view.rs` | Left Pane 320/Compact 48、Toggle 40×40、Item 高 40/icon 16/字 14、选中指示条 3×16 强调色、Header Margin 56,44,0,0、Top 模式 48 横排 | ✅ | ✅ `pane_widths` / `toggle_click_switches` / `select_top_item` / `content_rect_beside_pane` / `top_mode_horizontal` / `indicator_emitted_on_selected` / `compact_hides_labels` |
| §29 ColorPicker | MetroColorPicker | `color_picker.rs` | RGB/A 四滑轨（0..255）、拇指 10×10、预览块 44+2px 边、Hex 文本、Spectrum 阶梯 hue 带近似、垂直朝向 312..392 | ✅ | ✅ `default_color_and_hex` / `slider_geometry` / `click_updates_channel` / `click_same_value_no_change` / `drag_updates_continuously` / `hex_formats` / `render_emits_spectrum_sliders_preview` |
| §30 ParallaxView | MetroParallaxView | `parallax_view.rs` | shift = scroll×ratio、clamp 到 ±MaxShift（MaxShiftRatio×视口）、纯位移辅助 | ✅ | ✅ `shift_scales_with_ratio` / `shift_clamped` / `clamped_false_passes_through` / `content_rect_translates` |
| §31 AnimatedIcon | MetroAnimatedIcon | `animated_icon.rs` | dir_off/dir_on 正交 chevron、set_state 0.1s 插值、几何插值渲染 | ✅ | ✅ `set_state_triggers_anim` / `same_state_no_op` / `render_emits_triangle` / `morph_interpolates_tip` |
| §32 SwipeControl | MetroSwipeControl | `swipe_control.rs` | Reveal 模式、Left/Right 操作项、拖出吸合/回弹、Execute 即触发、点项 Invoke/点内容 Close | ✅ | ✅ `drag_reveals_left` / `drag_below_threshold_bounces` / `click_invoke_returns_index` / `click_content_closes` / `execute_mode_resets_after_drag` / `clamp_limits_offset` / `render_emits_revealed_items` |
